...
ArchMarshal 0.14 is a safety-hardened alpha, not yet a stable product. This page
...
| Existing project adoption | Implemented, review-first | Root-bound ownership; additive default/workspace/explicit Skill roots; exact package paths/bytes/modes/hashes and complete backup coverage bound in the v2 plan; one backup-through-publication lifetime lock; verified backup; durable create-only journal; receipt-last forward recovery; changed targets block and are never replaced | Handle-relative no-follow filesystem backend; orphan transaction inspection |
