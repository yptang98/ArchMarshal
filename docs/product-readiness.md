...
|---|---|---|---|
| Codex plugin experience | Implemented | Validated uniquely named repository marketplace and plugin manifest; natural-language Skill is the primary entrypoint; matching engine is resolved from the configured full Git marketplace snapshot and its version, API, exact file set, and source-tree hash must match the installed plugin lock; ambient engine-package fallback is forbidden; update candidates are validated while the old version stays enabled; active Python is the dependency fast path; a bounded commit-last capsule supports rollback or temporary local recovery; runtime pointers cannot block bootstrap identity checks | Public installation telemetry, richer host-native progress UI, signed marketplace/release provenance, and a host-native atomic marketplace swap primitive |
